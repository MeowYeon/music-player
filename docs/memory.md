...
- 产品名：聆听
- v0.5：MVP 阶段正式结束
- v0.6：主界面重做已完成，进入成熟产品方向
- 当前界面：无右栏主界面、可折叠导航、全局搜索、歌曲工作区、歌单工作台、我喜欢、最近播放、媒体库卡片、移动迷你播放器
- 后端：仍保留 Go + SQLite + 本地扫描 + 音频流基础；最近播放接口具备 `lastPlayedAt` 时间语义
- 前端：Vite + React + TypeScript，页面结构已拆出应用壳、共享歌曲列表和队列组件
