...
v0.4 在 v0.3 的媒体库和歌曲库基础上补齐基础播放器能力：普通歌单、系统歌单、前端播放队列和播放顺序。
...
系统歌单固定存在，不能删除和重命名。`我喜欢` 用于收藏，`最近播放` 由播放行为自动维护。普通歌单只按添加时间排序，不做手动排序。

详细功能设计见 [v0.4 基础播放器功能设计](./v0.4-player.md)。

### 播放队列与播放顺序

播放队列在 v0.4 先作为前端内存状态实现：
...
播放队列刷新页面后丢失。播放模式支持顺序播放、列表循环、随机播放。歌曲真正开始播放后，前端调用后端接口记录到 `最近播放`。
...
侧边栏展开时图标和文本并排展示，收起时只展示图标。歌曲管理、普通歌单详情、我喜欢、最近播放中的歌曲行交互保持一致：播放按钮、单击选中、双击播放、收藏按钮、三点菜单。

详细界面说明见 [v0.4 UI 设计](./v0.4-ui.md)。
