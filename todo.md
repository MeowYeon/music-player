...
- [ ] 底部播放器进度条需要接入真实播放进度，而不是固定 `0`。
- [ ] 支持拖动进度条跳转播放位置。
- [ ] 播放/暂停状态需要完全跟随 audio 事件，处理加载失败、播放结束、用户暂停等情况。
- [ ] 上一首、下一首按钮当前没有行为，需要基于当前歌曲列表实现顺序切歌。
- [ ] 当前歌曲时长为 `0:00` 时观感较弱，需要在 UI 上区分“未知时长”和真实 `0:00`。
- [ ] 点击歌曲后应有加载状态，避免大文件或慢磁盘时用户不知道是否开始播放。

### 歌曲列表

- [ ] 歌曲表格缺少空值展示规则，artist/album 为空时应显示更友好的占位，例如 `未知艺术家`、`未知专辑`。
- [ ] 当前行选中只表达“当前歌曲”，还需要区分播放中、暂停中、加载中。
- [ ] 搜索时没有 loading/error/empty 状态的细分展示。
- [ ] 歌曲列表数量增大后需要考虑滚动区域高度和表头固定。
- [ ] 文件名 fallback 的标题中可能包含 `歌名 - 艺术家`，后续可在前端或扫描阶段做轻量拆分。

### 扫描任务面板

- [ ] 扫描完成后右侧当前任务区域会消失，只剩最近任务；需要更清晰地保留“最近一次扫描结果”。
- [ ] 开始扫描后按钮只显示创建中，缺少“扫描中”禁用状态和防重复提交策略。
- [ ] 扫描失败时右侧需要更明显的错误提示，并保留失败目录路径。
- [ ] 最近任务列表需要限制高度并滚动，避免任务多时挤压页面。
- [ ] 对已扫描目录再次扫描时，应在 UI 上提示这是“重新扫描此目录”，不是新增目录。

### 顶部与导航

- [ ] 左侧导航当前只剩“歌曲”，如果短期没有更多页面，可以考虑进一步压缩侧栏宽度或移除侧栏。
- [ ] 顶部媒体库摘要需要在扫描完成后稳定刷新，避免用户误以为扫描没有生效。
- [ ] 搜索框需要增加清空按钮。
- [ ] 顶部和右侧区域在小屏宽度下需要响应式折叠规则。

### 状态与错误

- [ ] 后端不可用时，前端需要显示明确错误，而不是只有空列表。
- [ ] API 请求失败时需要统一错误提示样式。
- [ ] 扫描路径为空、目录不存在、无权限时，需要在表单下方显示具体错误。
- [ ] 无歌曲、扫描中、扫描完成但无音频文件三种空状态需要文案区分。

### 视觉细节

- [ ] 当前页面已经比 mock 简洁，但还需要根据真实数据检查文字截断、列宽和路径换行。
- [ ] 右侧长路径应中间省略，避免撑破面板。
- [ ] 表格行 hover、active、selected 状态需要统一。
- [ ] 图标按钮需要补齐 tooltip 或 aria label，提升可用性。

### v0.2 验收标准

- [ ] 打开页面后，不依赖 mock，能清楚看到后端连接状态。
- [ ] 扫描 `/mnt/c/Users/guohp/Music/test` 后，右侧能展示完成结果，顶部摘要和歌曲表同步更新。
- [ ] 点击任意歌曲后，底部播放器显示加载、播放、暂停、结束状态。
- [ ] 播放进度随 audio 更新，拖动进度条可跳转。
- [ ] 上一首、下一首可在当前列表内切换。
- [ ] 空值、错误、空列表、扫描失败都有明确页面反馈。
